...
class CommHandler:
	...
	def stop_and_decend():
		...
		fanleft    = motorDict[str(MOTOR_LEFT)]
		fanright   = motorDict[str(MOTOR_RIGHT)]
		servoleft  = motorDict[str(MOTOR_LEFT_SERVO)]
		servoright = motorDict[str(MOTOR_RIGHT_SERVO)]
	...
	def process_get_pwm(self):
		...
		for num, m in motorDict.iteritems():
